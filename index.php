...
<body>
    <nav>
        <ul>
            <li class="theme" id="blocks">
                blocks
                <span class="tooltiptext">Kattints rá a téma kiválasztásához!</span>
            </li>
            <li class="theme" id="items">
                items
                <span class="tooltiptext">Kattints rá a téma kiválasztásához!</span>
            </li>
            <li class="theme" id="mobs">
                mobs
                <span class="tooltiptext">Kattints rá a téma kiválasztásához!</span>
            </li>
            <li class="theme" id="foods">
                foods
                <span class="tooltiptext">Kattints rá a téma kiválasztásához!</span>
            </li>
            <li>toplist</li>
    ...
    <div id="sizeContainer">
        <button class="card" id="4x4">
            4x4
            <span class="tooltiptext">Kattints rá a méret kiválasztásához!</span>
        </button>
        <button class="card" id="4x6">
            4x6
            <span class="tooltiptext">Kattints rá a méret kiválasztásához!</span>
        </button>
        <button class="card" id="6x6">
            6x6
            <span class="tooltiptext">Kattints rá a méret kiválasztásához!</span>
        </button>
        <button class="card" id="6x8">
            6x8
            <span class="tooltiptext">Kattints rá a méret kiválasztásához!</span>
        </button>
    </div>
...
<script>
    ...
    function SetupGame(Size){
        ...
        ImagesByTheme = [...sources[theme]]; // getting every image of the current theme
        Images = [];

        for (i = 0; i < Width * Height / 2; i++){
            randomImageIND = Math.floor(Math.random() * ImagesByTheme.length);
            Images[i] = ImagesByTheme[randomImageIND];
            Images[i + Width * Height / 2] = ImagesByTheme[randomImageIND];
            ImagesByTheme.splice(randomImageIND, 1);
        }
    ...
    function CreateButtons(width, height){
        ...
        for (y = 0; y < height; y++){
            ...
            for (x = 0; x < width; x++){
                ...
                card_back_img.src = Images[randomBG];
                Images.splice(randomBG, 1);
                card_back.classList.add("card_face");
                card_back.classList.add("back");
                card_inner.appendChild(card_back);
                
                card_back_img.onload = (e) =>{

                    
                    let canvas = document.createElement("canvas");
                    let ratio = (parseFloat(card.style.height.slice(card.style.height, -2)) / 1.5) / 16;
                    canvas.width = parseFloat(card.style.height.slice(card.style.height, -2)) / 1.5;
                    canvas.height = 16*ratio;

                    let context = canvas.getContext("2d");
                    context.drawImage(card_back_img, 0, 0, canvas.width, canvas.height);

                    let newImageURL = context.canvas.toDataURL("image/jpeg", 10);
                    let new_Image = document.createElement("img");
                    new_Image.src = newImageURL;
                    card_back.appendChild(new_Image);
                };
                
                card_back_img.style.width = parseFloat(card.style.height.slice(card.style.height, -2)) / 1.5 + "px";
                card_back_img.style.height = parseFloat(card.style.height.slice(card.style.height, -2)) / 1.5 + "px";
    ...
    function CheckPair(div){
        ...
        else if (div.children[1].children[0].src == LastClickedBtn.children[1].children[0].src){ // ha megegyezik a két kiválasztott kártya
            div.flippable = false;
        ...
        else if (div.children[1].children[0].src != LastClickedBtn.children[1].children[0].src) { // ha nem egyezik meg a két -||-
            Points -= 20;
